...
-- Tạo user TEST_USER nếu chưa có
BEGIN
    EXECUTE IMMEDIATE 'CREATE USER TEST_USER IDENTIFIED BY test123';
    EXECUTE IMMEDIATE 'GRANT CONNECT, CREATE SESSION TO TEST_USER';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
DECLARE
    v_success BOOLEAN;
    v_columns column_array := column_array('MAGV', 'LUONG');
BEGIN
    -- GRANT SELECT một số cột trên bảng QLDT_GIAOVIEN
    QLTDH.GRANT_SELECT_TO_USER_OR_ROLE('TEST_USER', 'TABLE', 'QLTDH.QLDT_GIAOVIEN', v_columns, FALSE, v_success);
    DBMS_OUTPUT.PUT_LINE('GRANT SELECT (MAGV, LUONG): ' || CASE WHEN v_success THEN 'THÀNH CÔNG' ELSE 'THẤT BẠI' END);

    -- GRANT UPDATE toàn bảng QLDT_THAMGIA
    QLTDH.GRANT_UPDATE_TO_USER_OR_ROLE('TEST_USER', 'TABLE', 'QLTDH.QLDT_THAMGIA', NULL, FALSE, v_success);
    DBMS_OUTPUT.PUT_LINE('GRANT UPDATE QLDT_THAMGIA: ' || CASE WHEN v_success THEN 'THÀNH CÔNG' ELSE 'THẤT BẠI' END);

    -- GRANT DELETE bảng QLDT_DETAI
    QLTDH.GRANT_DELETE_TO_USER_OR_ROLE('TEST_USER', 'TABLE', 'QLTDH.QLDT_DETAI', FALSE, v_success);
    DBMS_OUTPUT.PUT_LINE('GRANT DELETE QLDT_DETAI: ' || CASE WHEN v_success THEN 'THÀNH CÔNG' ELSE 'THẤT BẠI' END);

    -- GRANT INSERT bảng QLDT_PHONGBAN
    QLTDH.GRANT_INSERT_TO_USER_OR_ROLE('TEST_USER', 'TABLE', 'QLTDH.QLDT_PHONGBAN', FALSE, v_success);
    DBMS_OUTPUT.PUT_LINE('GRANT INSERT QLDT_PHONGBAN: ' || CASE WHEN v_success THEN 'THÀNH CÔNG' ELSE 'THẤT BẠI' END);

    -- GRANT EXECUTE thủ tục PROC_CAPNHAT_PHUCAP
    QLTDH.GRANT_EXEC_TO_USER_OR_ROLE('TEST_USER', 'QLTDH.PROC_CAPNHAT_PHUCAP', FALSE, v_success);
    DBMS_OUTPUT.PUT_LINE('GRANT EXEC PROC_CAPNHAT_PHUCAP: ' || CASE WHEN v_success THEN 'THÀNH CÔNG' ELSE 'THẤT BẠI' END);

    -- GRANT EXECUTE thủ tục PROC_DSTHAMGIA_GV
    QLTDH.GRANT_EXEC_TO_USER_OR_ROLE('TEST_USER', 'QLTDH.PROC_DSTHAMGIA_GV', FALSE, v_success);
    DBMS_OUTPUT.PUT_LINE('GRANT EXEC PROC_DSTHAMGIA_GV: ' || CASE WHEN v_success THEN 'THÀNH CÔNG' ELSE 'THẤT BẠI' END);

    -- GRANT EXECUTE function FN_TONG_THOIGIAN_THAMGIA
    QLTDH.GRANT_EXEC_TO_USER_OR_ROLE('TEST_USER', 'QLTDH.FN_TONG_THOIGIAN_THAMGIA', FALSE, v_success);
    DBMS_OUTPUT.PUT_LINE('GRANT EXEC FN_TONG_THOIGIAN_THAMGIA: ' || CASE WHEN v_success THEN 'THÀNH CÔNG' ELSE 'THẤT BẠI' END);
END;
/

SELECT object_name, object_type
FROM all_objects
WHERE owner = 'TEST_USER' -- Thay USER_NAME bằng tên user thực tế
